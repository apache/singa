...
template <typename T> static inline T byteSwap(const T &v) {
  int size = sizeof(v);
  T ret;
...
template <typename T> static inline T hton(const T &v) {
  return isNetworkOrder() ? v : byteSwap(v);
}

template <typename T> static inline T ntoh(const T &v) { return hton(v); }
