...
namespace singa{
  ...
  class Tensor {
    ...
    %rename(LT_op_float) operator<(const Tensor &t, const float x);
    //%rename(LT_op_int) operator<(const Tensor &t, const int x);
    //
    %rename(LE_op_float) operator<=(const Tensor &t, const float x);
    //%rename(LE_op_int) operator<=(const Tensor &t, const int x);
    //
    %rename(GT_op_float) operator>(const Tensor &t, const float x);
    //%rename(GT_op_int) operator>(const Tensor &t, const int x);
    //
    %rename(GE_op_float) operator>=(const Tensor &t, const float x);
    //%rename(GE_op_int) operator>=(const Tensor &t, const int x);
    //

    %rename(Add_T) operator+(const Tensor &lhs, const Tensor &rhs);
    %rename(Sub_T) operator-(const Tensor &lhs, const Tensor &rhs);
    %rename(Mul_T) operator*(const Tensor &lhs, const Tensor &rhs);
    %rename(Div_T) operator/(const Tensor &lhs, const Tensor &rhs);
