...
//ww: it would be better to give a description of each variable, e.g., posdata_,dstposdata_. All *_ varible should be declared in *.h file.
void DBMBottomLayer::Setup(const LayerProto& proto,    /*possrc(vdim): data of data layer; posdata_inter(vdim): u0, here u0 equals v; posdata(hdim): Wv; dstposdata(hdim): u1*/
  ...
  CHECK_EQ(srclayers.size(),1);   // ww: only one srcslayer, why you use two in the next two lines
  ...
  posdata_inter_.Reshape(vector<int>{batchsize_, vdim_}); // ww: inter_ means sample?
  negdata_inter_.Reshape(vector<int>{neg_batchsize_, vdim_}); // ww: inter_ means sample?
  negdata_prob_.Reshape(vector<int>{neg_batchsize_, vdim_});   /*probabaility,,?????????should all be declared here?*/
  dstposdata_.ReshapeLike(posdata_);                 /*u from dstlayer*/ //ww: is dst for the upper layer in this Big Layer
  dstnegdata_.reshapeLike(negdata_);                 /*h from dstlayer*/
  Factory<Param>* factory=Singleton<Factory<Param>>::Instance();
  weight_=shared_ptr<Param>(factory->Create("Param"));
  bias_=shared_ptr<Param>(factory->Create("Param"));
  weight_->Setup(proto.param(0), vector<int>{vdim_, hdim_}, vdim_*hdim_); //ww: why three arguments?
...
void DBMBottomLayer::ComputeFeature(bool positive, const vector<SLayer>& srclayers) {
  if (positive){
    //ww: 1. get input data into lower layer in this Big Bottom Layer 2 multiply it with weight matrix and assing to upper layer in this Big Bottom Layer
	...
	Tensor<cpu, 1> bias(bias_->mutable_cpu_data(), Shape1(vdim_)); //ww: not used?
        // repmat: repeat bias vector into batchsize rows
	posdata_inter=possrc; //ww: why assign to posdata_inter
        posdata=dot(posdata_inter, weight) /*to dstlayer*/
  }
  else{
    //ww: 1. mulitply upper layer with weight matrix + bias->sigmoid ->assign to lower layer in this Big Layer
    /*negative compute feature*/
        ...
        negdata_prob=dot(dstnegdata, weight.T()); // ww:: where did you define dstnegdata?
        // repmat: repeat bias vector into batchsize rows
        ...
        negdata=dot(negdata_inter, weight) /* to dstlayer*/ //ww:
  }
}
...
void DBMMiddleLayer::ComputeFeature(bool positive, const vector<SLayer>& srclayers) {
  if (positive){
	...
	if(srclayers[0]->mutable_dstposdata(this)!=nullptr){  /*to source layer*/ //ww: no need to check this, since it is a MiddelLayer
