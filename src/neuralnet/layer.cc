...
void DBMBottomLayer::Setup(const LayerProto& proto,
  ...
  batchsize_=possrc.shape()[0];                        /*positive and negetive data has the same batchsize?*/
...
void DBMBottomLayer::ComputeFeature(bool positive, const vector<SLayer>& srclayers) {
  if (positive){
        Tensor<cpu, 2> posdata(posdata_.mutable_cpu_data(), Shape2(batchsize_,hdim_));  /*possrc: vdim, posdata_inter: vdim, posdata: hdim, posdata = dot(posdata_inter, weight), dstposdata: hdim, dstposdata = sigmoid (posdata+...)*/
	...
	posdata_inter=possrc;
        ...
        Tensor<cpu, 2> negdata(negdata_.mutable_cpu_data(), Shape2(neg_batchsize_,hdim_));  /*possrc: vdim, posdata_inter: vdim, posdata: hdim, posdata = dot(posdata_inter, weight), dstposdata: hdim, dstposdata = sigmoid (posdata+...)*/
        ...
        negdata_inter = (float)rand()%10/10 > negdata_prob ? 0 : 1; /*gibbs sampling!!!!!! but not traversei + no realization for k-step persistent?*/
...
void DBMTopLayer::ComputeFeature(bool positive, const vector<SLayer>& srclayers) {
  if (positive){                                       /*possrc: vdim, posdata_inter: vdim, posdata: hdim, posdata = dot(posdata_inter, weight), dstposdata: hdim, dstposdata = sigmoid (posdata+...)*/
        Tensor<cpu, 2> posdata_inter(posdata_inter_.mutable_cpu_data(), Shape2(batchsize_,vdim_));
	CHECK_EQ(srclayers[0]->posdata(this).count(), batchsize_*vdim_);
        Tensor<cpu, 2> possrc(srclayers[0]->mutable_posdata(this)->mutable_cpu_data(),
         Shape2(batchsize_,vdim_));
        ...
        /*possrc: vdim, posdata_inter: vdim, posdata: hdim, posdata = dot(posdata_inter, weight), dstposdata: hdim, dstposdata = sigmoid (posdata+...)*/
        Tensor<cpu, 2> negdata_prob(negdata_prob_.mutable_cpu_data(), Shape2(neg_batchsize_,vdim_));  /*sampling probability, is the blob float?*/
        Tensor<cpu, 2> negdata_inter(negdata_inter_.mutable_cpu_data(), Shape2(neg_batchsize_,vdim_));  /*sampling value*/
        CHECK_EQ(srclayers[0]->negdata(this).count(), neg_batchsize_*vdim_);
        Tensor<cpu, 2> negsrc(srclayers[0]->mutable_negdata(this)->mutable_cpu_data(),
