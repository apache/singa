...
% module core_tensor % include "config.i" % include "std_vector.i" %
    include "std_string.i" % include "std_shared_ptr.i"
...
% apply(float *IN_ARRAY1, int DIM1){(const float *src, const size_t num)} %
    apply(int *IN_ARRAY1, int DIM1){(const int *src, const size_t num)} %
    apply(float *ARGOUT_ARRAY1, int DIM1){(float *value, const size_t num)} %
...
enum DataType { kFloat32, kFloat16, kInt, kChar, kDouble };

inline size_t Product(const std::vector<size_t> &shape, int start = 0,
                      size_t len = 0);
...
class Tensor {
 public:
  ...
  Tensor(const std::vector<size_t> &shape, std::shared_ptr<singa::Device> dev,
  ...
  void CopyDataFromHostPtr(const DType *src, const size_t num,
  ...
  void RepeatData(std::vector<size_t> repeats, int axis, int total_repeats,
                  const Tensor &src);
...
void RepeatDataToFrom(bool broadcast_flag, std::vector<size_t> repeats,
...
template <typename SType>
void AddColumn(const SType alpha, const SType beta, const Tensor &v, Tensor *M);
