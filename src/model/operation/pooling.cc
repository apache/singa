...
PoolingHandle::PoolingHandle(const Tensor &input,
  ...
  if (input.device()->lang() == kCpp) {
    ...
    y_md =
        new mkldnn::memory::desc({y_dims}, dtype, mkldnn::memory::format::nchw);

    // allow max or avg (follow cudnn implementation convention)
    pooling_algo = mkldnn::pooling_avg_exclude_padding;
    if (is_max_pooling) pooling_algo = mkldnn::pooling_max;

    pool_fwd_d = new mkldnn::pooling_forward::desc(
        mkldnn::forward_training, pooling_algo, *x_md, *y_md, s_dims, k_dims,
    ...
    if (is_max_pooling) {
      ...
      //    (mkldnn_forward_training) and backward
      //    (mkldnn_backward) passes to save indices where maximum was found.
      //    Workspace layout is opaque and
      //    the indices cannot be restored from it. However one can use backward
      //    pooling to perform up-sampling
...
PoolingHandle::~PoolingHandle() {
  ...
  if (x_md == nullptr) {
    ...
    delete (pool_fwd_d);
    delete (pool_fwd_pd);
    if (is_max_pooling) delete (ws_mem);
  }
#endif  // USE_MKLDNN
}
  ...
  y.device()->Exec(
      [&y, &x, &ph](Context *ctx) {
        ...
        try {
          ...
          auto x_mem =
              memory({{{ph.x_dims}, ph.dtype, memory::format::nchw}, eng},
                     x.block()->mutable_data());

          auto p_fwd =
              ph.is_max_pooling
                  ? pooling_forward(*ph.pool_fwd_pd, x_mem, y_mem, *ph.ws_mem)
                  : pooling_forward(*ph.pool_fwd_pd, x_mem, y_mem);

          stream(stream::kind::eager).submit({p_fwd}).wait();
        } catch (mkldnn::error &e) {
          LOG(FATAL) << "MKLDNN pooling fwd"
  ...
  in_grad.device()->Exec(
      [&in_grad, &grad, &ph](Context *ctx) {
        try {
          auto eng = *ctx->engine;
          using namespace mkldnn;
          auto pool_bwd_d = pooling_backward::desc(
              ph.pooling_algo, *ph.x_md, *ph.y_md, ph.s_dims, ph.k_dims,
              ph.p_dims, ph.p_dims, padding_kind::zero);
          ...
          auto dy_mem = memory(
              {{{ph.y_dims}, memory::data_type::f32, memory::format::nchw},
               eng},
              grad.block()->mutable_data());

          auto p_bwd =
              ph.is_max_pooling
                  ? pooling_backward(pool_bwd_pd, dy_mem, *ph.ws_mem, dx_mem)
                  : pooling_backward(pool_bwd_pd, dy_mem, dx_mem);

          stream(stream::kind::eager).submit({p_bwd}).wait();
        } catch (mkldnn::error &e) {
          LOG(FATAL) << "MKLDNN pooling bwd"
                     << "Status: " << e.status << " Message: " << e.message;
        }
