...
BatchNormHandle::BatchNormHandle(const float momentum, const Tensor &input) {
  ...
  if (input.device()->lang() == kCpp) {
    ...
    epsilon = 1e-5f;
    data_memory_format =
        is_2d ? mkldnn::memory::format::nc : mkldnn::memory::format::nchw;
    if (is_2d) {
    ...
    bn_fwd_d = new mkldnn::batch_normalization_forward::desc(
        mkldnn::forward_training, *x_md, epsilon, mkldnn::use_scale_shift);
    bn_fwd_pd =
        new mkldnn::batch_normalization_forward::primitive_desc(*bn_fwd_d, eng);
  }
#endif // USE_MKLDNN
};
  ...
  y.device()->Exec(
      [&y, &x, &running_mean, &running_var, &w, &bnh](Context *ctx) {
        try {
          ...
          auto x_mem =
              memory({{{bnh.x_dims}, bnh.dtype, bnh.data_memory_format}, eng},
                     x.block()->mutable_data());
          auto y_mem =
              memory({{{bnh.y_dims}, bnh.dtype, bnh.data_memory_format}, eng},
          ...
          auto bn_fwd_d = batch_normalization_forward::desc(
              forward_inference, *bnh.x_md, bnh.epsilon, flags);
          auto bn_fwd_pd =
          ...
          auto bn = batch_normalization_forward(
              bn_fwd_pd, x_mem, (const primitive::at)m_mem,
              (const primitive::at)v_mem, w_mem, y_mem);

          stream(stream::kind::eager).submit({bn}).wait();
        } catch (mkldnn::error &e) {
          InitLogging("");
          LOG(FATAL) << "MKLDNN Batch Norm "
...
CpuBatchNormForwardTraining(const BatchNormHandle &bnh, const Tensor &x,
  ...
  y.device()->Exec(
      [&x, &y, &mean, &var, &w, &bnh](Context *ctx) {
        try {
          auto eng = *ctx->engine;
          using namespace mkldnn;
          ...
          auto bn_fwd = batch_normalization_forward(*bnh.bn_fwd_pd, x_mem,
                                                    w_mem, y_mem, m_mem, v_mem);

          stream(stream::kind::eager).submit({bn_fwd}).wait();
        } catch (mkldnn::error &e) {
          singa::InitLogging("");
          LOG(FATAL) << "MKLDNN Batch Norm Backward"
                     << "Status: " << e.status << " Message: " << e.message;
        }
      },
...
const std::vector<Tensor>
CpuBatchNormBackwardx(const BatchNormHandle &bnh, const Tensor &y,
                      const Tensor &dy, const Tensor &x, const Tensor &bnScale,
                      const Tensor &bnBias, const Tensor &mean,
                      const Tensor &var) {
  Tensor dx;
  dx.ResetLike(dy);
      ...
      [&dw, &x, &dx, &y, &dy, &w, &mean, &var, &bnh](Context *ctx) {
        ...
        try {
          ...
          auto bn_bwd_d = batch_normalization_backward::desc(
              backward, *bnh.dx_md, *bnh.x_md, bnh.epsilon, use_scale_shift);
          auto bn_bwd_pd = batch_normalization_backward::primitive_desc(
              bn_bwd_d, eng, *bnh.bn_fwd_pd);

          auto dw_mem = memory(bn_bwd_pd.diff_weights_primitive_desc(),
                               dw.block()->mutable_data());

          auto bn_bwd = batch_normalization_backward(
              bn_bwd_pd, x_mem, m_mem, v_mem, dy_mem, w_mem, dx_mem, dw_mem);
...
const std::vector<Tensor>
GpuBatchNormForwardTraining(const CudnnBatchNormHandle &cbnh, const Tensor &x,
                            const Tensor &bnScale, const Tensor &bnBias,
                            Tensor &running_mean, Tensor &running_var) {
...
const std::vector<Tensor>
GpuBatchNormBackward(const CudnnBatchNormHandle &cbnh, const Tensor &dy,
                     const Tensor &x, const Tensor &bnScale, const Tensor &mean,
                     const Tensor &var) {
