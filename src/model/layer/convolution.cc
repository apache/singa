...
const Tensor Convolution::Forward(int flag, const Tensor &input) {
  ...
  for (size_t b = 0; b < batchsize; b++) {
    Im2col(in_data + b * imagesize, channels_, height_, width_, kernel_h_,
           ...
           conv_height_, conv_width_, data_col);
...
Convolution::Backward(int flag, const Tensor &grad) {
  ...
  for (size_t b = 0; b < batchsize; b++) {
    ...
    Col2im(dcol_data, channels_, height_, width_, kernel_h_, kernel_w_, pad_h_,
           pad_w_, stride_h_, stride_w_, conv_height_, conv_width_, dx_b);
    ...
    for (int h = 0; h < conv_h; ++h) {
      for (int w = 0; w < conv_w; ++w) {
        ...
        if (h_pad >= 0 && h_pad < height && w_pad >= 0 && w_pad < width)
          data_col[(c * conv_h + h) * conv_w + w] =
              data_im[(c_im * height + h_pad) * width + w_pad];
        else
          data_col[(c * conv_h + h) * conv_w + w] = 0;
      }
    }
  }
}
...
void Convolution::Col2im(const float *data_col, const int channels,
                         ...
                         const int stride_h, const int stride_w,
                         const int conv_h, const int conv_w, float *data_im) {
