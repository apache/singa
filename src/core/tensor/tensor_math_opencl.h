...
void Abs<float, lang::Opencl>(const size_t num, const Block* in, Block* out, Context* ctx) {
  auto ocl_ctx = get_context(ctx->vcl_ocl_ctx_id);
...
void Sigmoid<float, lang::Opencl>(const size_t num, const Block* in, Block* out, Context* ctx) {
  ...
  const viennacl::vector<float> zero = viennacl::zero_vector<float>(num, ctx->vcl_ctx);
  const viennacl::vector<float> one = viennacl::scalar_vector<float>(num, 1.0f, ctx->vcl_ctx);
...
/*  cl_int status = CL_SUCCESS;
  ...
  status = kernel.setArg(0, outbuf);
  status = kernel.setArg(1, seed);
  status = kernel.setArg(2, low);
  status = kernel.setArg(3, high);
  status = kernel.setArg(4, rounds);
  status = kernel.setArg(5, cl_uint(num) / 4);
...
void Nrm2<float, lang::Opencl>(const size_t num, const Block* in, float* out, Context* ctx) {
  ...
  auto res = viennacl::linalg::norm_2(inbuf);
  out[0] = res;
}
