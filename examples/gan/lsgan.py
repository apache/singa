...
class LSGAN():
    ...
    def __init__(self,
        ...
        self.dis_net_relu_0 = layer.Activation(
            ...
            mode='relu',
            input_sample_shape=(self.hidden_size,))
        self.dis_net_fc_1 = layer.Dense(name='d_fc_1',
                                        num_output=1,
                                        use_bias=True,
    ...
    def train(self):
        ...
        opt_0 = optimizer.Adam(
            lr=self.learning_rate)  # optimizer for discriminator
        opt_1 = optimizer.Adam(
            lr=self.learning_rate
        )  # optimizer for generator, aka the combined model
        ...
        for epoch in range(self.epochs):
            for d_step in range(self.d_steps):
                ...
                grads, (d_loss_real,
                        _) = self.dis_net.train(real_imgs, substrahend)
                for (s, p, g) in zip(self.dis_net.param_names(),
                                     self.dis_net.param_values(), grads):
                    opt_0.apply_with_lr(epoch, self.learning_rate, g, p, str(s),
                                        epoch)
                substrahend.set_value(-1.0)
                grads, (d_loss_fake,
                        _) = self.dis_net.train(fake_imgs, substrahend)
                for (s, p, g) in zip(self.dis_net.param_names(),
                                     self.dis_net.param_values(), grads):
                    opt_0.apply_with_lr(epoch, self.learning_rate, g, p, str(s),
                                        epoch)
            ...
            for g_step in range(self.g_steps):
                ...
                for (s, p, g) in zip(self.gen_net.param_names(),
                    ...
                    opt_1.apply_with_lr(epoch, self.learning_rate, g, p, str(s),
                                        epoch)

            if epoch % self.interval == 0:
                self.save_image(epoch)
                print_log('The {} epoch, G_LOSS: {}, D_LOSS: {}'.format(
                    epoch, g_loss, d_loss))
...
if __name__ == '__main__':
    ...
    lsgan = LSGAN(dev, rows, cols, channels, noise_size, hidden_size, batch,
                  interval, learning_rate, epochs, d_steps, g_steps,
