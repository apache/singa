...
def p1_evaluate_query(space_name, dataset, run_id, N, K) -> (list, float):
    ...
    """
    arch_id, candidates, current_time = fetch_from_db(space_name, dataset,
                                                      run_id, N)
...
class RunPhase1:
    ...
    def __init__(self,
        ...
        if self.search_space_ins.name == Config.MLPSP and self.N >= min(
                len(self.search_space_ins), 100000):
            print("Explore all models")
            strategy = SequenceSampler(self.search_space_ins)
        elif self.search_space_ins.name != Config.MLPSP and self.N >= min(
                len(self.search_space_ins), 8000):
            print("Explore all models")
            strategy = SequenceSampler(self.search_space_ins)
        else:
            strategy = RegularizedEASampler(
    ...
    def run_phase1(self) -> (list, list, list, list):
        ...
        while explored_n <= self.N:
            ...
            if arch_id is None:
                break
            model_encoding = self.search_space_ins.serialize_model_encoding(
                arch_micro)
            ...
            if explored_n % 100 == 0:
                logger.info("3. [trails] Phase 1: filter phase explored " +
                            str(explored_n) + " model, model_id = " +
                            model_eva.model_id + " model_scores = " +
                            json.dumps(model_eva.model_score))
            ...
            ranked_score = self.sampler.fit_sampler(
                model_eva.model_id,
            ...
            if len(trace_highest_score) == 0:
                ...
                if ranked_score > trace_highest_score[-1]:
                    ...
                    trace_highest_scored_models_id.append(
                        trace_highest_scored_models_id[-1])
