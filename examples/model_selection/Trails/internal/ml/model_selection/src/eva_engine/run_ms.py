...
class RunModelSelection:
    ...
    def select_model_simulate(self,
                              budget: float,
                              run_id: int = 0,
        ...
        K, U, N, B1_planed_time, B2_planed_time, B2_all_epoch = coordinator.schedule(
            self.dataset, self.sh, budget, score_time_per_model,
            train_time_per_epoch, run_workers, self.search_space_ins, N_K_ratio,
            only_phase1)

        print(f"Budget = {budget}, N={N}, K={K}")

        # 2. run phase 1 to score N models
        k_models, B1_actual_time_use = p1_evaluate_query(
            self.search_space_name, self.dataset, run_id, N, K)

        # 3. run phase-2 to determine the final model
        best_arch, best_arch_performance, B2_actual_epoch_use, _ = self.sh.run_phase2(
            U, k_models)
    ...
    def select_model_online_clean(self,
                                  budget: float,
                                  data_loader: List[DataLoader],
                                  only_phase1: bool = False,
                                  run_workers: int = 1):
        """
        ...
        K, U, N = self.coordination(budget, score_time_per_model,
                                    train_time_per_epoch, only_phase1)
    ...
    def select_model_online(self,
        ...
        logger.info(
            f"0. [trails] Begin model selection, is_simulate={self.is_simulate} ... "
        )
        begin_time = time.time()
        ...
        p1_runner = RunPhase1(args=self.args,
                              K=K,
                              N=N,
                              search_space_ins=self.search_space_ins,
                              train_loader=train_loader,
    ...
    def profile_filtering(self,
                          data_loader: List[DataLoader] = [None, None, None]):
        ...
        score_time_per_model = self.search_space_ins.profiling_score_time(
            ...
            is_simulate=self.is_simulate)
        logger.info(
            f"0. [trails] profile_filtering Done, time_usage = {time.time() - begin_time}"
        )
        return score_time_per_model

    def profile_refinement(self,
                           data_loader: List[DataLoader] = [None, None, None]):
        ...
        train_time_per_epoch = self.search_space_ins.profiling_train_time(
            ...
            is_simulate=self.is_simulate)
        logger.info(
            f"0. [trails] profile_refinement Done, time_usage = {time.time() - begin_time}"
        )
        return train_time_per_epoch

    def coordination(self, budget: float, score_time_per_model: float,
                     train_time_per_epoch: float, only_phase1: bool):
        logger.info("1. [trails] Begin coordination...")
        begin_time = time.time()
        sh = BudgetAwareControllerSH(search_space_ins=self.search_space_ins,
                                     dataset_name=self.dataset,
                                     eta=self.eta,
                                     time_per_epoch=train_time_per_epoch,
                                     is_simulate=self.is_simulate,
        ...
        K, U, N, B1_planed_time, B2_planed_time, B2_all_epoch = coordinator.schedule(
            self.dataset, sh, budget, score_time_per_model,
            train_time_per_epoch, 1, self.search_space_ins, n_k_ratio,
            only_phase1)

        logger.info(
            f"1. [trails] Coordination Done, time_usage = {time.time() - begin_time}"
        )
        return K, U, N
    ...
    def filtering_phase(self, N, K, train_loader=None):
        ...
        k_models, all_models, p1_trace_highest_score, p1_trace_highest_scored_models_id \
            = p1_runner.run_phase1()
        logger.info(
            f"2. [trails] filtering_phase Done, time_usage = {time.time() - begin_time}"
        )
        print(
            f"2. [trails] filtering_phase Done, time_usage = {time.time() - begin_time}"
        )
    ...
    def refinement_phase(self,
        ...
        if alg_name == Config.SUCCHALF:
            ...
            self.sh = UniformAllocation(search_space_ins=self.search_space_ins,
                                        dataset_name=self.dataset,
                                        eta=self.eta,
                                        time_per_epoch=train_time_per_epoch,
                                        args=self.args)
        ...
        best_arch, best_arch_performance, B2_actual_epoch_use, total_time_usage = self.sh.run_phase2(
            U, k_models)
        logger.info(
