...
def sampler_args(parser):
    ...
    parser.add_argument('--simple_score_sum',
                        default='True',
                        type=str2bool,
...
def space201_101_share_args(parser):
    parser.add_argument('--api_loc',
                        type=str,
                        default="NAS-Bench-201-v1_1-096897.pth",
    ...
    parser.add_argument('--init_channels',
                        default=16,
                        type=int,
                        help='output channels of stem convolution')
...
def nb101_args(parser):
    parser.add_argument('--num_stacks',
                        default=3,
                        type=int,
                        help='#stacks of modules')
    parser.add_argument('--num_modules_per_stack',
                        default=3,
                        type=int,
                        help='# modules per stack')
...
def nb201_args(parser):
    parser.add_argument(
        ...
        default='none',
        help=
        'weight initialization (before pruning) type [none, xavier, kaiming, zero]'
    )
    parser.add_argument(
        '--init_b_type',
        type=str,
        default='none',
        help=
        'bias initialization (before pruning) type [none, xavier, kaiming, zero]'
    )
...
def mlp_args(parser):
    ...
    parser.add_argument(
        ...
        default=20,
        type=int,
        help='number of hidden layer choices, 10 for criteo, 20 for others')
...
def seq_train_all_params(parser):
    ...
    parser.add_argument('--total_models_per_worker',
                        type=int,
                        default=-1,
                        help='How many models to evaluate')
    parser.add_argument(
        '--pre_partitioned_file',
        default=
        "./internal/ml/model_selection/exps/sampled_data/sampled_models_all.json",
        type=str,
        help='all models with id')


def dis_train_all_models(parser):
    parser.add_argument('--worker_each_gpu',
                        default=6,
                        type=int,
                        help='num worker each gpu')
...
def tune_interval_NK_rate(parser):
    parser.add_argument('--kn_rate',
                        default=-1,
                        type=int,
                        help="default N/K = 100")
...
def anytime_exp_set(parser):
    ...
    parser.add_argument('--is_simulate',
                        default='True',
                        type=str2bool,
...
def system_performance_exp(parser):
    parser.add_argument('--models_explore',
                        default=10,
                        type=int,
                        help='# models to explore in the filtering phase')
    parser.add_argument('--tfmem',
                        default="jacflow",
                        type=str,
                        help='the matrix t use, all_matrix')
    parser.add_argument('--embedding_cache_filtering',
                        default='True',
                        type=str2bool,
    ...
    parser.add_argument('--concurrency',
                        default=1,
                        type=int,
                        help='number of worker in filtering phase')
